    ...
    public class StorageOnMap : MonoBehaviour
    {
        ...
        private void AddItemsOnQueue()
        {
            ...
            if (emptyStorages.Count != 0)
            {
                ...
                {
                    _itemQueue.AddRange(_itemsOnMapController.ItemsList.Where(x => x.Item.ID == storage.StorageData.ItemType.ID).ToList());
                }
        ...
        {
            _currentStorage = _storageControllers.FirstOrDefault(x => x.StorageData.ItemType.ID == _currentItem.Item.ID);
        ...
        private void ChangeCharacterState(ItemData itemData)
        {
            ...
            {
               var itemsList = _itemQueue.Where(x => x.Item.ID == controller.StorageData.ItemType.ID).ToList();
            ...
            var storage = _storageControllers.FirstOrDefault(x => x.StorageData.ItemType.ID == itemData.ID);
            if (storage == null || storage.IsFullStorage())
            {
                return;
            }
            _itemQueue.AddRange(_itemsOnMapController.ItemsList.Where(x => x.Item.ID == itemData.ID && x.IsEnable).ToList());
